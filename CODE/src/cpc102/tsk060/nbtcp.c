 ...
 * $Author: garyg $
 * $Locker:  $
 * $Date: 2017/02/23 21:34:10 $
 * $Id: nbtcp.c,v 1.25 2017/02/23 21:34:10 garyg Exp $
 * $Revision: 1.25 $
 * $State: Exp $
 */
...
int Send_model_data = FALSE;  /* TRUE when sending Model Data */
...
static void Check_read_activity (int read_poll_status, int user_type); 
static void Close_sockets (int user_type);
static void Get_file_naming_convention ();
static void Get_current_time (struct tm *current_time);
...
/********************************************************************************
     ...
     Description: This routine monitors the read activity and closes the sockets
                  if read timer has expired
...
#define READ_EXPIRATION_TIME 200
#define SEND_KEEPALIVE_TIME  140
...
static void Check_read_activity (int read_poll_status, int user_type) {
   ...
   if (read_poll_status > 0)
      last_read_time = current_time;
   else {
      delta_time = current_time - last_read_time;
      /* if we haven't received a msg within x seconds,
         start sending keepalive msgs to see if we receive any ACKs */
      if ((delta_time - (time_t) SEND_KEEPALIVE_TIME) >= 0) {
         if ((delta_time % 10) == 0) { /* send keepalive msg every 10 secs */
            SM_send_keepalive_msg (user_type);
            sprintf (Log_buf, "Sending Keep-alive msg at activity delta time: %d\n", (uint) delta_time);
            MA_printlog (Log_buf);
         }
         if (delta_time > READ_EXPIRATION_TIME) {
            sprintf (Log_buf, "Read activity timer expired, closing sockets\n");
            MA_printlog (Log_buf);
            Close_sockets (user_type);
            /* Reset last_read_time */
            last_read_time = current_time;
         }
      }
   }
...
/********************************************************************************
     ...
     Description: This routine closes the socket
...
void Close_sockets (int user_type) {

   SOC_close_sockets ();
   Link_connected = FALSE;

   if (user_type == CLASS_1) {
      printf ("\nConnection lost, attempting to reconnect\n");

      sprintf(Log_buf, "Connection to ORPG %s Failed\n  Attempting to reconnect\n",
              SOC_get_inet_addr()); 
...
/********************************************************************************
     ...
     Description: This routine get the current time
 ...
 ********************************************************************************/
 ...
 void Get_current_time (struct tm *current_time) {
    ...
    gmtime_r ((const time_t *) &t, current_time);
...
static void Run_native_interface () {
          ...
          int poll_ret;
           struct tm local_utc;
           struct tm model_data_sent_time;
   static int in_background = 0;
   ...
   for(;;) {
      ...
      if(Link_connected == FALSE)  {
         ...
         if ((poll_ret = SOC_pollsocks()) == -1) {
            Close_sockets (CLASS_1);
         } else
            Check_read_activity (poll_ret, CLASS_1);
         ...
         if( !in_background )
            TERM_check_input (Link_connected, Interface_selected, Orpg_port);
             ...
             /* See if we need to send Model Data */
          if (Send_model_data == TRUE) {
             Get_current_time (&local_utc);

                /* Send Model Data on the top of each hour */
             if ((local_utc.tm_hour != model_data_sent_time.tm_hour)  && 
                 (local_utc.tm_min == 0)  && (local_utc.tm_sec == 0)) { 
                    SM_send_model_data_msg ();
                    model_data_sent_time = local_utc;
             }
          }

              /* this sleep throttles the rate at which products are read. 
                 A one second delay is enough to eventually cause the products 
                 to back up in the output queue on the RPG */
...
/********************************************************************************
     ...
     Description: This routine runs an interface/menu just for WAN OTRs
...
static void Run_otr_interface () {
   ...
   for(;;) {
      ...
      if ((poll_ret = SOC_pollsocks()) == -1 && !LoopOTRs) {
          Close_sockets (CLASS_2);
