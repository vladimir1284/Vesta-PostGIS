 ...
 * $Author: garyg $
 * $Locker:  $
 * $Date: 2017/02/23 21:34:11 $
 * $Id: nbtcp_process_rpg_msgs.c,v 1.19 2017/02/23 21:34:11 garyg Exp $
 * $Revision: 1.19 $
 * $State: Exp $
 */
...
#define AWIPS_PRODUCT_COMPLETION_TIME  15

int PROC_process_msg (char *buf, int msg_len)
{
          int  ret;
   msg_header  *msghead;
          int    product_timer_expired = 0;
   ...
   static time_t product_start_time;
   ...
   if ((buf == NULL) && (msg_len == -1)) {
      if (prodfd != -1) {
         ...
         prodfd = -1;
         msg_segmented = FALSE;
         sprintf (Log_msg, "Removing segmented file \"%s\" due to lost connection\n", filepathname);
         MA_printlog (Log_msg);
      }
      return (0);
   }
   ...
   if (msg_segmented == FALSE) {
      ...
      sprintf(Log_msg, "First product segment received (product length: %d)\n", prod_length);
      MA_printlog (Log_msg);

      product_start_time = time (NULL);
...
/*      if (New_connection_made == TRUE || msg_code == GSM_CODE) {
         sprintf(Log_msg, "New connection made flag: %d; msg_code = %d; mode flag = %d\n", 
                 New_connection_made, msg_code, mode);
         MA_printlog (Log_msg);
      } */
   ...
   if (((time_t) time(NULL) - product_start_time) >= AWIPS_PRODUCT_COMPLETION_TIME)
      product_timer_expired = 1;
   ...
   if((bytes_processed >= prod_length) && !product_timer_expired) {
      if (Otr_only)
         printf ("Done.\n");
      if (prodfd != -1) {
         if((close(prodfd)) < 0){
...
//         printf ("Fin...\n");
      ...
      sprintf(Log_msg, "Last product segment received (segment length: %d)\n", msg_len);
      MA_printlog (Log_msg);
      ...
      if (product_timer_expired) {
         SOC_close_sockets ();
         sprintf(Log_msg, "Closing sockets - AWIPS product completion timer expired\n");
         MA_printlog (Log_msg);
         msg_segmented = FALSE;
      } else {
/*         sprintf(Log_msg, "Intermediate product segment received (segment length: %d)\n", msg_len);
         MA_printlog (Log_msg); */
      }
   }
...
static void Add_to_list( char *filename ){
   ...
   if( node == NULL ){
      ...
      char text[128];
      sprintf( &text[0], "malloc of %zu bytes Failed\n", sizeof(Product_file_t) );
      MA_abort(text);
