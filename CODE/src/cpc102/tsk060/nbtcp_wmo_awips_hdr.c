 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2018/03/30 19:51:13 $
 * $Id: nbtcp_wmo_awips_hdr.c,v 1.11 2018/03/30 19:51:13 steves Exp $
 * $Revision: 1.11 $
 * $State: Exp $
 */
...
static void Set_distribution( char *icao, char *distribution );
...
int WAH_add_awips_wmo_hdr ( char *icao )
{
   ...
   if( icao != NULL )
      Set_distribution( icao, &WMO_hdr.distribution[0] );
   else
...
static void Init_awips_hdr()
{
   ...
   for( i = 0; i < MAX_PRODS; i++ ){
      ...
      switch (i) {
         ...
         case 82:	/* Supplemental Precipitation Data. */
         {
            memcpy( &Nodes[i].cat_prod.category[0], "SPD", 3 );
            break;
         }
         ...
         case 177:	/* Hydrometeor Classification (HHC). */
         {
            memcpy( &Nodes[i].cat_prod.category[0], "HHC", 3 );
            break;
         }

         case 181:	/* TWDR Base Reflectivity. */
         {
            /* 0.1-0.8 deg. */
            memcpy( &Nodes[i].cat_prod.category[0], "TR0", 3 );
            Nodes[i].cat_prod.elev_based = 1;
            Nodes[i].cat_prod.elev_min = 1;
            Nodes[i].cat_prod.elev_max = 8;

            node = Add_node( i, 10, 10, "TR1" );
            Nodes[i].next = node;
            
            node1 = Add_node( i, 16, 37, "TR2" );
            node->next = node1;
            
            node1 = Add_node( i, 38, 195, "TR?" );
            node->next = node1;
            break;
         }

         case 182:	/* TWDR Base Velocity. */
         {
            /* 0.1-0.8 deg. */
            memcpy( &Nodes[i].cat_prod.category[0], "TV0", 3 );
            Nodes[i].cat_prod.elev_based = 1;
            Nodes[i].cat_prod.elev_min = 1;
            Nodes[i].cat_prod.elev_max = 8;

            node = Add_node( i, 10, 10, "TV1" );
            Nodes[i].next = node;
            
            node1 = Add_node( i, 16, 37, "TV2" );
            node->next = node1;
            
            node1 = Add_node( i, 38, 195, "TV?" );
            node->next = node1;
            break;
         }

         case 186:	/* TWDR Long Range Reflectivity. */
         {
            /* 0.6 deg. */
            memcpy( &Nodes[i].cat_prod.category[0], "TLZ", 3 );
            Nodes[i].cat_prod.elev_based = 1;
            Nodes[i].cat_prod.elev_min = 6;
            Nodes[i].cat_prod.elev_max = 6;
            break;
...
static Node_t* Add_node( int code, int elev_min, int elev_max, char *cat )
{
   ...
   if( node == NULL ){
      ...
      char text[128];
      sprintf( &text[0], "calloc Failed for %zu bytes\n", sizeof(Node_t) );
      MA_abort(text);
...
/*********************************************************************************
   ...
   Description:
      Sets distribution based on ICAO.
...
static void Set_distribution( char *icao, char *distribution ){
   ...
   /* Go thru all radar names to find a match. */
   while( strlen( Radar_sites[i].radar ) > 0 ){  
   

      if( strstr( Radar_sites[i].radar, icao ) != NULL ){
      
         memcpy( distribution, Radar_sites[i].distribution, 2 ); 
         printf( "WMO Distribution Code for Site %s: %s\n", icao, Radar_sites[i].distribution );
         return;
