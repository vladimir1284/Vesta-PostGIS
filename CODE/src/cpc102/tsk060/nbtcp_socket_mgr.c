 ...
 * $Author: garyg $
 * $Locker:  $
 * $Date: 2016/11/17 19:51:04 $
 * $Id: nbtcp_socket_mgr.c,v 1.16 2016/11/17 19:51:04 garyg Exp $
 * $Revision: 1.16 $
 * $State: Exp $
 */
                                    ...
                                    char *password, int user_class)
{
   ...
   if(connect(PVC0_sock, (struct sockaddr *)&Sinaddr, sizeof(Sinaddr)) == -1)
   {
      ...
      if (previous_error != error) {
          fprintf (stdout, "...Attempting to connect (err: %d)\n", error);
          sprintf(Log_msg, "PVC0 connect error (%s - errno: %d)\n", 
   ...
   if (Class == CLASS_1) {
      ...
      if(connect(PVC1_sock, (struct sockaddr *)&Sinaddr, sizeof(Sinaddr)) == -1)
      {
         ...
         if (previous_error != error) {
             ...
             sprintf(Log_msg, "PVC1 connect error (%s - errno: %d)\n", 
                     strerror (error), error);
             MA_printlog(Log_msg);
             previous_error = error;
         }
   ...
   MA_printlog(Log_msg);
      ...
      /* log on with login message (link id, #pvcs, pvc#, password) 
         on each pvc - should receive acknowledgement message on pvc0 */
...
/********************************************************************************
     ...
     Description: This routine polls the sockets for activity and services
          ...
          Return: -1 if a poll or read error occurs; 
                   0 if there is nothing to read; 
                   postive integer for the number of poll events read
...
int SOC_pollsocks()
{
    ...
    if (Class == CLASS_1)
       poll_ret = poll(psock, 2, 1000); /* poll with 1 sec timeout */
    else
       poll_ret = poll(psock, 1, 1000); /* poll with 1 sec timeout */
    ...
    if(psock[0].revents != 0) {
       if((psock[0].revents) & (POLLERR | POLLHUP | POLLNVAL | POLLRDHUP)) return(-1);
       if ((Read_sock (PVC0)) ==-1) return (-1);
    }

    if (Class == CLASS_1) {
       if(psock[1].revents != 0) {
          if((psock[1].revents) & (POLLERR | POLLHUP | POLLNVAL | POLLRDHUP)) return(-1);    
